...
public class MRecordarme
{
	...
	public boolean buscaRecordarmeByUid(long uid)
	{
		try
		{
			PreparedStatement ps = conexion.prepareStatement("SELECT id, token, salt, uid FROM tresenraya.recordarme WHERE uid = ?");
			ps.setLong(1, uid);
	...
	public boolean buscaRecordarmeById(String id)
	{
		try
		{
			PreparedStatement ps = conexion.prepareStatement("SELECT id, token, salt, uid FROM tresenraya.recordarme WHERE id = ?");
			ps.setString(1, id);
	...
	public void crearRecordarme(String id, String token, String salt, long uid)
	{
		try
		{
			PreparedStatement ps = conexion.prepareStatement("INSERT INTO tresenraya.recordarme (id, token, salt, uid) VALUES (?, ?, ?, ?)");
			ps.setString(1, id);
	...
	public void actualizaRecordarme(String id, String token, String salt, long uid)
	{
		try
		{
			PreparedStatement ps = conexion.prepareStatement("UPDATE tresenraya.recordarme SET token = ?, salt = ?, uid = ? WHERE id = ?");
	...
	public void borrarRecordarme(String id)
	{
		try
		{
			PreparedStatement ps = conexion.prepareStatement("DELETE FROM tresenraya.recordarme WHERE id = ?");
			ps.setString(1, id);
			ps.executeUpdate();
