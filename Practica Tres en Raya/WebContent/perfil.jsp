...
<%
	request.setAttribute("titulo-pagina", "Mi Perfil");
...
<%	
	...
	Usuario usuario = (Usuario) s.usuario;
	if (usuario == null)
		response.sendRedirect("login.jsp");
	...
	boolean estaUsuarioConectado = mdlUsuariosConectados.estaUsuarioConectado(usuario.uid);
%>
	...
	<%@ include file="layout/header.jsp" %>
		...
		<div class="container">
			<div class="row">
				<secion id="mi-perfil" ng-controller="perfilCtrl" class="col-sm-7 col-sm-offset-1 col-xs-12">
					
					<div id="perfil-datos-seguridad">
						<h2>Datos de Seguridad</h2>
						
						<% if (estaUsuarioConectado) { %>
							<p class="text-success">Actualmente conectado</p>
						<% } else { %>
							<p><strong>Última conexión: </strong><%= fechaConexion.getTime() %></p>
						<% } %>

						<p>Dirección IP de la última conexión: <strong><%= usuario.ip %></strong>.</p>
					</div>
					...
					<div id="perfil-datos-usuario" class="col-sm-6 col-xs-12">

						<h2>Datos de Usuarios</h2>
						
						<p>Registrado el <%= fechaRegistro.getTime() %></p>
						
						<div class="alert alert-warning">
							<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
							<strong>Atención</strong>
							<span>Tenga en cuenta que si cambia su dirección de correo electrónico o contraseña, su sesión actual quedará anulada y tendrá que activar su cuenta de nuevo.</span>
						</div>

						<div class="alert alert-success" ng-show="success">
							<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
							<strong>Éxito</strong> <span>Ha modificado su perfil correctamente.</span>
						</div>

						<form>
							<div class="form-group">
								<label for="nombre">Nombre de usuario</label>
								<input class="form-control" type="nombre" name="nombre" ng-model="usuario.nombre" placeholder="Nombre de usuario" disabled>
							</div>

							<div class="form-group">
								<label for="correo">Correo</label>
								<input class="form-control" type="correo" name="correo" ng-model="usuario.correo" placeholder="Nombre de usuario">
								<div class="alert alert-danger" ng-show="error.correo">
									<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
									<strong>Error!</strong>
									<span ng-show="error.correo.vacio">Debe insertar un correo obligatoriamente</span>
									<span ng-show="error.correo.formato">El formato de correo no es válido</span>
								</div>
							</div>
							...
							<div class="form-group">
								...
								<select class="form-control" name="pais" id="pais" ng-model="usuario.pais">
							...
							<div class="form-group">
								<label for="antigua-contrasena">Antigua Contraseña</label>
								<input class="form-control" type="password" name="antigua-contrasena" ng-model="usuario.oldContrasena">
								<small>(Introduzca únicamente si va a cambiar la contraseña)</small>
								<div class="alert alert-danger" ng-show="error.contrasena.oldEquals">
									<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
									<strong>Error!</strong>
									<span>La contraseña que ha introducido no es correcta</span>
								</div>
							</div>

							<div class="form-group">
								<label for="nueva-contrasena">Nueva Contraseña</label>
								<input class="form-control" type="password" name="nueva-contrasena" ng-model="usuario.newContrasena" placeholder="Nueva contraseña">
								<div class="alert alert-danger" ng-show="error.contrasena.underflow || error.contrasena.overflow || error.contrasena.vacio">
									<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
									<strong>Error!</strong>
									<span ng-show="error.contrasena.underflow">La contraseña que ha introducido es demasiado corta (mínimo 6 caracteres)</span>
									<span ng-show="error.contrasena.overflow">La contraseña que ha introducido es demasiado larga (máximo 50 caracteres)</span>
									<span ng-show="error.contrasena.vacio">Debe introducir una contraseña obligatoriamente</span>
								</div>
							</div>

							<div class="form-group">
								<label for="confirmar-nueva-contrasena">Nueva Contraseña</label>
								<input class="form-control" type="password" name="confirmar-nueva-contrasena" ng-model="usuario.confirmContrasena" placeholder="Confirmar nueva contraseña">
								<div class="alert alert-danger" ng-show="error.contrasena.newEquals">
									<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
									<strong>Error!</strong>
									<span ng-show="error.contrasena.newEquals">La contraseña confirmada que ha introducido no coincide con su nueva contraseña</span>
								</div>
							</div>
							
							<div class="alert alert-warning" ng-show="activacion">
								<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
								<strong>Atención</strong>
								<span>Usted ha cambiado su correo o contraseña. Su cuenta ha sido desactivada por seguridad. Debe acceder a la dirección de correo <strong>{{ usuario.correo }}</strong> para confirmar su cuenta.</span>
							</div>

							<div class="form-group">
								<input type="button" ng-click="modificar()" name="btn-perfil" value="Modificar Perfil" class="btn btn-lg btn-success">
							</div>

							<div class="form-group">
								<a href="index.jsp" class="btn btn-md btn-warning">Cancelar Modificación</a>
							</div>
						</form>
					</div>
				</secion>
