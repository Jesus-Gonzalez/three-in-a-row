					...
					token != null && !token.isEmpty())
				{
					...
					if (mdlRecordarme.buscaRecordarmeById(identificador))
					{
						...
						if (cifrado.hash("SHA-256", mdlRecordarme.salt + token).equals(mdlRecordarme.token))
						{
							...
							MUsuariosConectados mdlUsrConectados = new MUsuariosConectados(conexion);
							mdlUsuarios.getUsuarioByUid(mdlRecordarme.uid);
							mdlUsuarios.getProximoUsuario();							
							
							Usuario usuario = new Usuario(mdlUsuarios);
							
							if (!mdlUsrConectados.estaUsuarioConectado(usuario.uid))
							{
								mdlUsrConectados.crearUsuarioConectado(usuario.uid, usuario.nombre, usuario.pais);
							}
...
//								mapaCookies.put("token", String.valueOf(numAleatorio));
							...
							response.addCookie(crearCookiePersistente("tresenraya", mapaCookies.toString()));
						}
			...
			} // Cookie 'tresenraya' no creada: no hacer nada
